...
func (h *PaymentHandler) CheckBalance(c fiber.Ctx) error {
	...
	return c.JSON(models.BalanceResponse{
		Balance: balance, Currency: req.Currency,
	})
}

func (h *PaymentHandler) GenerateAddress(c fiber.Ctx) error {
	fmt.Println("ddd")
	...
	address, encryptedPrivateKey, err := svc.GenerateAddress()
	if err != nil {
		logger.Error("Failed to Generate Address", err, "currency", req.Currency)
		return errors.HandleAPIError(c, err)
	}

	logger.Info("generate Address ", "currency", req.Currency, "address", address, "balance", 0)
	return c.JSON(generateAddressResponse{
		Address: address, EncryptedPrivateKey: encryptedPrivateKey, Currency: req.Currency,
	})
}
